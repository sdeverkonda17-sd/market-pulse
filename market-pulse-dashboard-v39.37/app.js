...
const NEWS_CACHE_PREFIX = 'market-pulse-news-v39.25:';
const SECTOR_CACHE_PREFIX = 'market-pulse-sector-v39.27:';
const UPDATES_CACHE_KEY = 'market-pulse-updates-v39.25';
const MAX_WATCHLIST_SIZE = 8;
const API_TIMEOUT_MS = 25000;
let reviewRequestId = 0;
const state = { leaders: [], selected: null, universeCount: 0, watchlist: [], watchRows: [], sector: 'banking', sectorLoaded: false, sectorData: null, moversLoaded: false, moversData: null, moversView: 'gainers', leaderPage: 0, sectorPage: 0, watchPage: 0 };
...
async function loadSector(sector = state.sector) {
  ...
  state.sectorPage = 0;
  const note = $('#sector-note'),table = $('#sector-table'),cacheKey=`${SECTOR_CACHE_PREFIX}${normalized}`,saved=readCache(cacheKey);
  document.querySelectorAll('[data-sector]').forEach(button => button.classList.toggle('is-active', button.dataset.sector === normalized));
  if(saved?.data){renderSectorLeaders({...saved.data,providerNotice:`Refreshing live data. ${saved.data.providerNotice||''}`});if(note)note.textContent=`${SECTOR_LABELS[normalized]} · saved ranking shown while live data refreshes…`;}
  else {if (note) note.textContent = `Loading ${SECTOR_LABELS[normalized]} sector data…`;if (table) table.innerHTML = '<tr><td colspan="9" class="connection-note">Loading the sector basket…</td></tr>';}
  try {
    const data=await api('sector', '', { sector: normalized });
    writeCache(cacheKey,{data,savedAt:new Date().toISOString()});
    renderSectorLeaders(data);
    state.sectorLoaded = true;
  } catch (error) {
    if(saved?.data){renderSectorLeaders({...saved.data,delayed:true,providerNotice:`Saved sector ranking is displayed because the live refresh failed: ${error.message}. Prices may be delayed.`});if(note)note.textContent=`${SECTOR_LABELS[normalized]} · saved data · live refresh unavailable`;state.sectorLoaded=true;}
    else {if (note) note.textContent = `${SECTOR_LABELS[normalized]} sector data is temporarily unavailable`;if (table) table.innerHTML = `<tr><td colspan="9" class="connection-note">Live providers did not return a usable basket: ${safe(error.message)}. <button class="review-button" data-sector="${safe(normalized)}" type="button">Retry sector</button></td></tr>`;}
  }
}
...
function renderUpdates(data) {
  ...
  list.innerHTML = updates.length
    ...
    : `<div class="updates-empty"><p>${safe(data.detail || 'No recent exchange announcements were returned.')}</p><a href="https://www.nseindia.com/companies-listing/corporate-filings-announcements" target="_blank" rel="noopener">Open official NSE corporate announcements</a></div>`;
}

async function loadUpdates() {
  const list = $('#updates-list'),saved=readCache(UPDATES_CACHE_KEY);
  if(saved?.data){renderUpdates({...saved.data,source:`${saved.data.source||'NSE updates'} · saved while refreshing`});}
  else if (list) list.innerHTML = '<p class="updates-empty">Loading exchange announcements…</p>';
  try {
    const data=await api('updates');
    writeCache(UPDATES_CACHE_KEY,{data,savedAt:new Date().toISOString()});
    renderUpdates(data);
  } catch (error) {
    if(saved?.data)renderUpdates({...saved.data,source:'Saved NSE updates · live refresh unavailable',detail:`Live refresh failed: ${error.message}`});
    else renderUpdates({ source: 'Live updates temporarily unavailable', detail: `The dashboard could not reach the updates function: ${error.message}. Use the official NSE link below.` });
  }
}
async function refresh() {
  ...
  try {
    ...
    if (cached?.leaders?.length) {
      ...
      const session=nseSessionState();
      $('#market-status').textContent = session.open ? `Live market feed unavailable — showing this device's saved ${source} result${saved ? ` from ${saved.toLocaleString()}` : ''}` : `NSE market closed — showing the last available ${source} snapshot${saved ? ` saved ${saved.toLocaleString()}` : ''}`;
...
async function openReview(symbol, mode = 'review') {
  ...
  try {
    ...
    if (cached?.raw?.stock) {
      ...
      $('#market-status').textContent = nseSessionState().open ? `Live feed unavailable — review uses this device's saved ${ticker} result` : `NSE market closed — review uses the last available ${ticker} snapshot`;
    } else if (fallback?.lastPrice) {
      fillReview({ stock: fallback, history: [], announcements: [] }, ticker, true, mode);
      $('#market-status').textContent = nseSessionState().open ? `Live feed unavailable — review uses the saved top-10 ${ticker} quote` : `NSE market closed — review uses the last available ${ticker} quote`;
    } else {
...
function openPaperEntryDialog(symbol,price){
  document.querySelector('#paper-entry-dialog')?.remove();
  const dialog=document.createElement('dialog');dialog.id='paper-entry-dialog';dialog.className='paper-entry-dialog';
  dialog.innerHTML=`<form method="dialog" class="paper-entry-form"><div><p class="research-label">PAPER TRADE</p><h3>Mark ${safe(symbol)} as entered</h3><p>This records a practice position only. It does not place an exchange order.</p></div><label>Entry price (₹)<input id="paper-entry-price" type="number" min="0.01" step="0.01" value="${Number(price).toFixed(2)}" required></label><label>Quantity<input id="paper-entry-quantity" type="number" min="1" step="1" value="1" required></label><p class="paper-entry-error" id="paper-entry-error" aria-live="polite"></p><div class="paper-entry-actions"><button value="cancel" class="ghost-button" type="button" data-paper-entry-cancel>Cancel</button><button class="primary-button" type="submit">Record paper entry</button></div></form>`;
  document.body.append(dialog);const form=dialog.querySelector('form');
  dialog.querySelector('[data-paper-entry-cancel]').addEventListener('click',()=>dialog.close());
  dialog.addEventListener('close',()=>dialog.remove());
  form.addEventListener('submit',event=>{event.preventDefault();const entry=Number(dialog.querySelector('#paper-entry-price').value),quantity=Math.floor(Number(dialog.querySelector('#paper-entry-quantity').value));if(!(entry>0)||!(quantity>0)){dialog.querySelector('#paper-entry-error').textContent='Enter a valid positive price and quantity.';return;}dialog.close();enterPaperTrade(symbol,entry,quantity);});
  dialog.showModal();dialog.querySelector('#paper-entry-price').select();
}
function enterPaperTrade(symbol,entryValue=null,quantityValue=null){
  const row=state.watchRows.find(item=>item.symbol===symbol),item=state.watchlist.find(value=>value.symbol===symbol)||{},metrics=row?.metrics,price=Number(metrics?.lastPrice||0);if(!price)return;
  if(entryValue===null||quantityValue===null){openPaperEntryDialog(symbol,price);return;}
  const entry=Number(entryValue),quantity=Math.floor(Number(quantityValue));if(!(entry>0)||!(quantity>0))return;const trades=paperTrades(),trade={status:'open',entryPrice:entry,quantity,enteredAt:new Date().toISOString()};trades[symbol]=trade;savePaperTrades(trades);const model=monitorStatus(item,metrics,trade),levels=`${item.stop?` Stop ${money.format(item.stop)}.`:' No stop is saved yet.'}${item.target?` Target ${money.format(item.target)}.`:' No target is saved yet.'}`;appendMonitorEvent(symbol,'IN_TRADE',entry,`Manual paper entry recorded: ${quantity} shares at ${money.format(entry)}.${levels} ${model.reason}`);renderWatchlist();
...
const headlineRequests=new Map();
async function headlineData(symbol){const ticker=cleanTicker(symbol),key=`${NEWS_CACHE_PREFIX}${ticker||'MARKET'}`;if(headlineRequests.has(key))return headlineRequests.get(key);const request=api('news',ticker).then(data=>{writeCache(key,{data,savedAt:new Date().toISOString()});return {...data,delayed:false};}).catch(error=>{const cached=readCache(key);if(cached?.data)return {...cached.data,delayed:true,note:`Saved headlines shown because live news could not be reached: ${error.message}`};throw error;}).finally(()=>window.setTimeout(()=>headlineRequests.delete(key),1000));headlineRequests.set(key,request);return request;}
function headlineFallback(symbol,error){const ticker=encodeURIComponent(`${symbol} NSE company news`),market=encodeURIComponent('India stock market global markets RBI crude oil earnings');return `<div class="news-fallback"><p>Live headline retrieval could not be reached (${safe(error.message)}). Missing news is not positive evidence.</p><a href="https://news.google.com/search?q=${ticker}&hl=en-IN&gl=IN&ceid=IN%3Aen" target="_blank" rel="noopener">Search ${safe(symbol)} company news</a><a href="https://news.google.com/search?q=${market}&hl=en-IN&gl=IN&ceid=IN%3Aen" target="_blank" rel="noopener">Check world and India market news</a></div>`;}function sellHeadlineScan(rows){const negative=['fraud','probe','investigation','penalty','default','downgrade','loss','decline','falls','weak','warning','resigns','resignation','cancelled','lawsuit','ban','tariff','war','conflict'],positive=['order win','wins order','approval','upgrade','profit rises','profit jumps','record profit','expansion','contract awarded'],titles=(rows||[]).map(row=>String(row.title||'').toLowerCase()),negativeHits=titles.filter(title=>negative.some(word=>title.includes(word))).length,positiveHits=titles.filter(title=>positive.some(word=>title.includes(word))).length;return {negativeHits,positiveHits,tone:negativeHits>positiveHits?'caution':positiveHits>negativeHits?'supportive':'neutral'};}
...
async function loadSellNewsPanels(symbol){const nodes=[...document.querySelectorAll('[data-sell-news]')].filter(node=>node.dataset.sellNews===symbol);if(!nodes.length)return;try{const data=await headlineData(symbol),companyScan=sellHeadlineScan(data.company),globalScan=sellHeadlineScan(data.global),warning=companyScan.tone==='caution'||globalScan.tone==='caution'?'Headline wording contains caution terms. Read the original reports before holding or selling.':'No concentration of obvious caution words was detected in the returned titles. This is not proof that the news is positive.';const html=`${data.delayed?'<p class="news-delay">Saved headline context is shown because the live feed is temporarily unavailable.</p>':''}<div class="sell-news-summary"><strong>News review: ${safe(warning)}</strong><span>Company titles: ${companyScan.negativeHits} caution / ${companyScan.positiveHits} supportive · World/India titles: ${globalScan.negativeHits} caution / ${globalScan.positiveHits} supportive</span></div><div class="sell-news-columns"><div><h4>${safe(symbol)} company news</h4>${sellNewsList(data.company)}</div><div><h4>World / India market news</h4>${sellNewsList(data.global)}</div></div><p class="method-note">The headline scan only identifies transparent keywords in titles; it does not understand the full article. News can trigger investigation, but never an automatic sell by itself.</p>`;nodes.forEach(node=>node.innerHTML=html);}catch(error){nodes.forEach(node=>node.innerHTML=headlineFallback(symbol,error));}}function sellAnalysis(metrics,item={},trade=null){
...
function stopWatching(symbol){
  const ticker=cleanTicker(symbol);if(!ticker||!isWatched(ticker))return;
  saveWatchlist(state.watchlist.filter(item=>item.symbol!==ticker));
  state.watchRows=state.watchRows.filter(item=>item.symbol!==ticker);
  state.expandedWatchSymbols?.delete(ticker);
  const sent=readCache(WATCH_ALERTS_KEY)||{};Object.keys(sent).filter(key=>key.startsWith(`${ticker}:`)).forEach(key=>delete sent[key]);writeCache(WATCH_ALERTS_KEY,sent);
  syncWatchButtons();renderWatchlist(state.watchRows);
  const note=$('#watchlist-note');if(note)note.textContent=`Stopped watching ${ticker}. Any paper-trade journal remains saved on this device.`;
}
function toggleWatch(symbol){const ticker=cleanTicker(symbol);if(!ticker)return;if(isWatched(ticker))stopWatching(ticker);else addOrUpdateWatch(ticker);}
...
function linearFit(values){
  const n=values.length;if(n<2)return {slope:0,start:values[0]||0,end:values.at(-1)||0};
  const meanX=(n-1)/2,meanY=average(values),den=values.reduce((sum,_,i)=>sum+(i-meanX)**2,0)||1;
  const slope=values.reduce((sum,value,i)=>sum+(i-meanX)*(value-meanY),0)/den,intercept=meanY-slope*meanX;
  return {slope,start:intercept,end:intercept+slope*(n-1)};
}
function detectTriangle(history){
  const rows=history.slice(-40).filter(row=>Number(row.high||row.close)>0&&Number(row.low||row.close)>0);
  if(rows.length<20)return {available:false,type:'No reliable triangle',status:'INSUFFICIENT HISTORY',explanation:'At least 20 daily candles with high and low values are required.'};
  const highs=rows.map(row=>Number(row.high||row.close)),lows=rows.map(row=>Number(row.low||row.close)),upper=linearFit(highs),lower=linearFit(lows),price=Number(rows.at(-1).close),scale=price||1,upperSlope=upper.slope/scale,lowerSlope=lower.slope/scale,startWidth=upper.start-lower.start,endWidth=upper.end-lower.end,convergence=startWidth>0?(startWidth-endWidth)/startWidth:0,tolerance=.012;
  const upperTouches=highs.filter((value,index)=>Math.abs(value-(upper.start+upper.slope*index))/scale<=tolerance).length,lowerTouches=lows.filter((value,index)=>Math.abs(value-(lower.start+lower.slope*index))/scale<=tolerance).length;
  let type='No reliable triangle';if(Math.abs(upperSlope)<.0015&&lowerSlope>.00045)type='Ascending triangle';else if(upperSlope<-.00045&&Math.abs(lowerSlope)<.0015)type='Descending triangle';else if(upperSlope<-.00035&&lowerSlope>.00035)type='Symmetrical triangle';
  const available=type!=='No reliable triangle'&&convergence>.18&&upperTouches>=2&&lowerTouches>=2&&endWidth>0,breakout=price>upper.end*1.003,breakdown=price<lower.end*.997,volumes=rows.map(row=>Number(row.volume||0)).filter(Boolean),volumeContracting=volumes.length>=15&&average(volumes.slice(-5))<average(volumes.slice(-15,-5)),confidence=Math.min(100,Math.round(35+Math.min(25,convergence*50)+Math.min(20,(upperTouches+lowerTouches)*3)+(volumeContracting?15:0))),status=!available?'NOT CONFIRMED':breakout?'BREAKOUT':breakdown?'FAILED / BREAKDOWN':'FORMING',direction=type==='Ascending triangle'?'bullish bias':type==='Descending triangle'?'bearish bias':'neutral until breakout';
  const explanation=available?type+' is '+status.toLowerCase()+'. Upper boundary is near '+money.format(upper.end)+' and lower boundary near '+money.format(lower.end)+'. It has '+upperTouches+' upper and '+lowerTouches+' lower-line touches; volume is '+(volumeContracting?'contracting during compression.':'not clearly contracting.'):'Recent highs and lows do not form a sufficiently converging structure with repeated touches, so the model refuses to label a triangle.';
  return {available,type,status,direction,confidence,upperStart:upper.start,upperEnd:upper.end,lowerStart:lower.start,lowerEnd:lower.end,upperTouches,lowerTouches,convergence,volumeContracting,breakout,breakdown,explanation,window:rows.length};
}
function detectOrb(intraday){
  const rows=(intraday||[]).filter(row=>Number(row.close)>0);if(rows.length<4)return {available:false,status:'NOT AVAILABLE',explanation:'ORB needs at least four completed 5-minute candles from the current trading session.'};
  const opening=rows.slice(0,3),rangeHigh=Math.max(...opening.map(row=>Number(row.high))),rangeLow=Math.min(...opening.map(row=>Number(row.low))),last=rows.at(-1),prior=rows.at(-2),averageVolume=average(rows.slice(-6,-1).map(row=>Number(row.volume||0)))||1,relativeVolume=Number(last.volume||0)/averageVolume,breakout=Number(last.close)>rangeHigh&&Number(last.close)>Number(last.open),breakdown=Number(last.close)<rangeLow,nearBreakout=!breakout&&Number(last.close)>=rangeHigh*.995,failed=Number(prior.high)>=rangeHigh&&Number(last.close)<rangeHigh,status=breakout&&relativeVolume>=1.2?'BREAKOUT CONFIRMED':breakout?'BREAKOUT — VOLUME WEAK':failed?'FALSE BREAKOUT':nearBreakout&&relativeVolume>=1.2?'BREWING':'INSIDE RANGE',score=Math.max(0,Math.min(100,Math.round((breakout?45:nearBreakout?25:5)+(relativeVolume>=1.5?30:relativeVolume>=1.2?20:5)+(failed?-30:0)+(breakdown?-25:0))));
  const explanation=status==='BREAKOUT CONFIRMED'?'A completed 5-minute candle closed above the 15-minute opening-range high of '+money.format(rangeHigh)+' with '+relativeVolume.toFixed(1)+'× recent volume.':status==='BREWING'?'Price is within 0.5% of the opening-range high and volume is '+relativeVolume.toFixed(1)+'× recent average, but no candle has closed above '+money.format(rangeHigh)+' yet.':status==='FALSE BREAKOUT'?'Price traded beyond '+money.format(rangeHigh)+' but closed back inside the opening range. Buyer follow-through has not held.':'Price remains between '+money.format(rangeLow)+' and '+money.format(rangeHigh)+'; there is no confirmed ORB entry.';
  return {available:true,status,score,rangeHigh,rangeLow,relativeVolume,breakout,breakdown,failed,explanation,candles:rows.slice(-36)};
}
function patternEvidence(history,intraday){const triangle=detectTriangle(history),orb=detectOrb(intraday);let score=0;if(triangle.available)score+=Math.round(triangle.confidence*.35);if(triangle.breakout)score+=15;if(triangle.breakdown)score-=20;if(orb.status==='BREAKOUT CONFIRMED')score+=35;else if(orb.status==='BREWING')score+=18;else if(orb.status==='FALSE BREAKOUT')score-=25;return {triangle,orb,score:Math.max(0,Math.min(100,score)),note:'Pattern evidence is a confirmation layer. It does not override fundamentals, market context, volume or risk limits.'};}
function historicalMetrics(stock){
  const history=(stock.history||[]).filter(point=>Number(point.close)>0).map(point=>({date:point.date,open:Number(point.open||point.close),high:Number(point.high||point.close),low:Number(point.low||point.close),close:Number(point.close),volume:Number(point.volume||0)})),closes=history.map(point=>point.close),lastPrice=Number(stock.lastPrice||closes.at(-1)||0),values=closes.length?closes:[lastPrice],ema20Series=emaSeries(values,20),ema50Series=emaSeries(values,50),ema20=ema20Series.at(-1)||lastPrice,ema50=ema50Series.at(-1)||lastPrice,reference=values.at(Math.max(0,values.length-21))||lastPrice,trend=reference?(lastPrice-reference)/reference*100:Number(stock.pChange||0),recent=values.slice(-20),support=recent.length?Math.min(...recent):Number(stock.dayLow||lastPrice),resistance=recent.length?Math.max(...recent):Number(stock.dayHigh||lastPrice),returns=values.slice(1).map((value,index)=>Math.log(value/values[index])).filter(Number.isFinite),volatility=returns.length?Math.sqrt(average(returns.map(value=>value*value)))*Math.sqrt(252)*100:0,rsi=rsiValue(values),base=Math.max(-35,Math.min(45,trend*1.8-volatility*.12)),patterns=patternEvidence(history,stock.intraday||[]);
  const scored=scoreStock({...stock,history,lastPrice,support,resistance,trend,volatility,rsi,ema20,ema50,ema20Series,ema50Series,base,bull:Math.min(65,base+14),bear:Math.max(-55,base-21)});
  return {...scored,intraday:stock.intraday||[],patterns,ml:{...mlOutlook(history),patternScore:patterns.score,patternEvidence:patterns.note}};
...
function watchButtonMarkup(symbol,compact=false){const ticker=cleanTicker(symbol),watched=isWatched(ticker),label=watched?'Stop watching':'Add to watchlist';return `<button class="watch-button${compact?' watch-icon-button':''}${watched?' is-watching':''}" data-watch="${safe(ticker)}" type="button" aria-pressed="${watched}" aria-label="${label} ${safe(ticker)}" title="${label}">${compact?(watched?'★':'☆'):(watched?'✓ Watching':'Watch')}</button>`;}
function syncWatchButtons(){document.querySelectorAll('[data-watch]').forEach(button=>{const watched=isWatched(button.dataset.watch),compact=button.classList.contains('watch-icon-button'),label=watched?'Stop watching':'Add to watchlist';button.classList.toggle('is-watching',watched);button.textContent=compact?(watched?'★':'☆'):(watched?'✓ Watching':'Watch');button.disabled=false;button.setAttribute('aria-pressed',String(watched));button.setAttribute('aria-label',`${label} ${button.dataset.watch}`);button.title=label;});}function stockCard(stock,index,sector=false){const scoreNote=stock.fundamentalScore===null?'technical context':'fundamentals + technicals',levelOrRisk=sector&&stock.support?`<div><span>Levels</span><strong>${money.format(stock.support)} / ${money.format(stock.resistance)}</strong><small>support / resistance</small></div>`:`<div><span>Risk</span><strong>${safe(stock.risk)}</strong><small>daily range ${stock.rangePct.toFixed(2)}%</small></div>`;return `<article class="stock-card"><div class="stock-card-heading"><div><span class="stock-rank">#${index+1}</span><strong>${safe(stock.symbol)}</strong><small>${safe(stock.name)}</small></div><span class="signal ${stock.signal}">${stock.signal}</span></div><div class="stock-card-metrics"><div><span>Last price</span><strong>${money.format(stock.lastPrice)}</strong><small class="${stock.pChange>=0?'positive':'negative'}">${percent(stock.pChange)} today</small></div><div><span>Signal score</span><strong>${stock.score}/100</strong><small>${scoreNote}</small></div>${levelOrRisk}</div><div class="stock-card-actions"><button class="review-button" data-review="${safe(stock.symbol)}" type="button">Review</button><button class="decision-button" data-decision="${safe(stock.symbol)}" type="button">Decision brief</button>${watchButtonMarkup(stock.symbol,sector)}</div></article>`;}function chartFrame(data,extraValues=[]){
  const width=700,height=300,pad=40,values=[...data.flatMap(row=>[Number(row.high||row.close),Number(row.low||row.close)]),...extraValues].filter(Number.isFinite),low=Math.min(...values),high=Math.max(...values),range=high-low||1,x=index=>pad+index*(width-pad*2)/Math.max(1,data.length-1),y=value=>height-pad-(value-low)*(height-pad*2)/range;
  const grid=[0,.25,.5,.75,1].map(step=>{const value=high-range*step,yy=y(value);return '<g><line x1="'+pad+'" y1="'+yy+'" x2="'+(width-pad)+'" y2="'+yy+'" class="chart-grid"/><text x="3" y="'+(yy+4)+'" class="chart-axis">'+money.format(value)+'</text></g>';}).join('');
  return {width,height,pad,low,high,range,x,y,grid};
}
function chart(metrics){
  const data=(metrics.history||[]).slice(-90);if(data.length<2)return '<p class="chart-empty">Historical OHLCV data was not returned, so switchable technical charts cannot be drawn yet.</p>';
  const ema20=(metrics.ema20Series||[]).slice(-data.length),ema50=(metrics.ema50Series||[]).slice(-data.length),frame=chartFrame(data,[...ema20,...ema50]),{width,height,pad,x,y,grid}=frame,points=values=>values.map((value,index)=>x(index).toFixed(1)+','+y(value).toFixed(1)).join(' '),dates=[0,Math.floor((data.length-1)/2),data.length-1].map(index=>'<text x="'+x(index)+'" y="'+(height-7)+'" text-anchor="middle" class="chart-axis">'+safe(String(data[index].date||'').slice(5))+'</text>').join('');
  const candleWidth=Math.max(2,Math.min(8,(width-pad*2)/data.length*.62)),candles=data.map((row,index)=>{const xx=x(index),open=y(row.open),close=y(row.close),high=y(row.high),low=y(row.low),top=Math.min(open,close),body=Math.max(1,Math.abs(close-open)),up=row.close>=row.open;return '<g class="'+(up?'candle-up':'candle-down')+'"><line x1="'+xx+'" x2="'+xx+'" y1="'+high+'" y2="'+low+'"/><rect x="'+(xx-candleWidth/2)+'" y="'+top+'" width="'+candleWidth+'" height="'+body+'"/></g>';}).join('');
  const maxVolume=Math.max(...data.map(row=>Number(row.volume||0)),1),volumeBars=data.map((row,index)=>{const h=Number(row.volume||0)/maxVolume*(height-pad*2),up=row.close>=row.open;return '<rect class="'+(up?'volume-up':'volume-down')+'" x="'+(x(index)-candleWidth/2)+'" y="'+(height-pad-h)+'" width="'+candleWidth+'" height="'+h+'"/>';}).join('');
  const triangle=metrics.patterns?.triangle||{},patternRows=data.slice(-Math.min(40,data.length)),patternFrame=chartFrame(patternRows,[triangle.upperStart,triangle.upperEnd,triangle.lowerStart,triangle.lowerEnd]),px=patternFrame.x,py=patternFrame.y,patternPrice=patternRows.map((row,index)=>px(index).toFixed(1)+','+py(row.close).toFixed(1)).join(' '),patternLines=triangle.available?'<line class="pattern-upper" x1="'+px(0)+'" y1="'+py(triangle.upperStart)+'" x2="'+px(patternRows.length-1)+'" y2="'+py(triangle.upperEnd)+'"/><line class="pattern-lower" x1="'+px(0)+'" y1="'+py(triangle.lowerStart)+'" x2="'+px(patternRows.length-1)+'" y2="'+py(triangle.lowerEnd)+'"/>':'';
  const orb=metrics.patterns?.orb||{},orbRows=orb.candles||[],orbFrame=orbRows.length?chartFrame(orbRows,[orb.rangeHigh,orb.rangeLow]):null,orbSvg=orbFrame?'<svg class="price-chart technical-svg" viewBox="0 0 700 300">'+orbFrame.grid+orbRows.map((row,index)=>{const xx=orbFrame.x(index),cw=8,o=orbFrame.y(row.open),c=orbFrame.y(row.close),hi=orbFrame.y(row.high),lo=orbFrame.y(row.low);return '<g class="'+(row.close>=row.open?'candle-up':'candle-down')+'"><line x1="'+xx+'" x2="'+xx+'" y1="'+hi+'" y2="'+lo+'"/><rect x="'+(xx-cw/2)+'" y="'+Math.min(o,c)+'" width="'+cw+'" height="'+Math.max(1,Math.abs(c-o))+'"/></g>';}).join('')+'<line class="orb-high" x1="40" x2="660" y1="'+orbFrame.y(orb.rangeHigh)+'" y2="'+orbFrame.y(orb.rangeHigh)+'"/><line class="orb-low" x1="40" x2="660" y1="'+orbFrame.y(orb.rangeLow)+'" y2="'+orbFrame.y(orb.rangeLow)+'"/></svg>':'<p class="chart-empty">Current-session 5-minute candles are unavailable. ORB will appear during or after a session when Upstox returns intraday data.</p>';
  return '<div class="advanced-chart multi-chart" data-multi-chart><div class="chart-view-tabs" role="tablist"><button class="chart-view-button is-active" data-chart-view="trend" type="button">Trend</button><button class="chart-view-button" data-chart-view="candles" type="button">Candles</button><button class="chart-view-button" data-chart-view="volume" type="button">Volume</button><button class="chart-view-button" data-chart-view="pattern" type="button">Triangle</button><button class="chart-view-button" data-chart-view="orb" type="button">ORB</button></div><div class="chart-panel is-active" data-chart-panel="trend"><div class="chart-legend"><span class="legend-price">Close</span><span class="legend-ema20">EMA 20</span><span class="legend-ema50">EMA 50</span><span class="legend-rsi">RSI '+valueOrDash(metrics.rsi)+'</span></div><svg class="price-chart technical-svg" viewBox="0 0 '+width+' '+height+'">'+grid+'<polyline points="'+points(data.map(row=>row.close))+'" class="price-line"/><polyline points="'+points(ema20)+'" class="ema-line ema20-line"/><polyline points="'+points(ema50)+'" class="ema-line ema50-line"/>'+dates+'</svg></div><div class="chart-panel" data-chart-panel="candles"><svg class="price-chart technical-svg" viewBox="0 0 '+width+' '+height+'">'+grid+candles+dates+'</svg></div><div class="chart-panel" data-chart-panel="volume"><svg class="price-chart technical-svg" viewBox="0 0 '+width+' '+height+'">'+volumeBars+dates+'</svg><p class="indicator-help">Volume bars show participation. Rising price with stronger volume is more convincing than a low-volume breakout.</p></div><div class="chart-panel" data-chart-panel="pattern"><div class="pattern-chart-heading"><strong>'+safe(triangle.type||'No reliable triangle')+'</strong><span>'+safe(triangle.status||'NOT AVAILABLE')+'</span></div><svg class="price-chart technical-svg" viewBox="0 0 700 300">'+patternFrame.grid+'<polyline points="'+patternPrice+'" class="price-line"/>'+patternLines+'</svg><p class="indicator-help">'+safe(triangle.explanation||'Pattern evidence is unavailable.')+'</p></div><div class="chart-panel" data-chart-panel="orb"><div class="pattern-chart-heading"><strong>15-minute Opening Range Breakout</strong><span>'+safe(orb.status||'NOT AVAILABLE')+'</span></div>'+orbSvg+'<p class="indicator-help">'+safe(orb.explanation||'ORB evidence is unavailable.')+'</p></div><p class="indicator-help"><strong>Use the view buttons:</strong> Trend compares EMAs, Candles shows open/high/low/close, Volume shows participation, Triangle draws converging boundaries only when validated, and ORB marks the first 15-minute range.</p></div>';
}
function bindChart(metrics){
  document.querySelectorAll('[data-multi-chart]').forEach(container=>container.addEventListener('click',event=>{const button=event.target.closest('[data-chart-view]');if(!button)return;container.querySelectorAll('[data-chart-view]').forEach(node=>node.classList.toggle('is-active',node===button));container.querySelectorAll('[data-chart-panel]').forEach(panel=>panel.classList.toggle('is-active',panel.dataset.chartPanel===button.dataset.chartView));}));
}
function patternMlPanel(metrics){
  const patterns=metrics.patterns||{},triangle=patterns.triangle||{},orb=patterns.orb||{};
  const conclusion=orb.status==='BREAKOUT CONFIRMED'?'ORB is confirmed, but entry still needs acceptable extension, market support and a defined stop.':orb.status==='BREWING'?'Opening activity is brewing near the ORB level, but the breakout candle has not confirmed.':triangle.breakout?'A triangle breakout is visible; volume and a successful retest should confirm whether it can hold.':triangle.available?'A triangle is forming, but price remains inside the structure.':'No sufficiently reliable triangle or confirmed ORB is available.';
  return '<section class="detail-section pattern-ml-panel"><p class="research-label">ML PATTERN EVIDENCE</p><h3>'+safe(conclusion)+'</h3><div class="pattern-score-grid"><div><span>Pattern score</span><strong>'+Number(patterns.score||0)+'/100</strong><small>confirmation evidence, not a buy score</small></div><div><span>ORB</span><strong>'+safe(orb.status||'Unavailable')+'</strong><small>'+safe(orb.available?Number(orb.relativeVolume||0).toFixed(1)+'× recent volume':'needs intraday candles')+'</small></div><div><span>Triangle</span><strong>'+safe(triangle.type||'None')+'</strong><small>'+safe(triangle.available?triangle.confidence+'% structural confidence':'strict checks not passed')+'</small></div></div><div class="pattern-explanations"><p><strong>ORB:</strong> '+safe(orb.explanation||'Unavailable.')+'</p><p><strong>Triangle:</strong> '+safe(triangle.explanation||'Unavailable.')+'</p></div><p class="method-note">'+safe(patterns.note||'Patterns are supporting evidence only.')+'</p></section>';
...
function fundamentalGrid(metrics){const f=metrics.fundamentals||{},source=f.available?'Upstox annual fundamentals':'Not available from current provider',period=(latest,previous)=>latest&&previous?`${latest} vs ${previous}`:'annual YoY';return `<section class="detail-section"><p class="research-label">FUNDAMENTAL CHECK</p><h3>Business quality and valuation context</h3><p class="method-note">${safe(source)}. “—” means the provider did not return that metric; it is not treated as 0% or as good news.</p><div class="fundamental-grid"><div><span>Revenue growth</span><strong>${valueOrDash(f.revenueGrowth,'%')}</strong><small>${safe(period(f.revenuePeriod,f.revenuePreviousPeriod))}</small></div><div><span>Profit growth</span><strong>${valueOrDash(f.profitGrowth,'%')}</strong><small>${safe(period(f.profitPeriod,f.profitPreviousPeriod))}</small></div><div><span>P/E</span><strong>${valueOrDash(f.pe,'x')}</strong><small>sector ${valueOrDash(f.sectorPe,'x')}</small></div><div><span>ROE</span><strong>${valueOrDash(f.roe,'%')}</strong><small>sector ${valueOrDash(f.sectorRoe,'%')}</small></div><div><span>ROCE</span><strong>${valueOrDash(f.roce,'%')}</strong><small>sector ${valueOrDash(f.sectorRoce,'%')}</small></div><div><span>Score inputs</span><strong>${metrics.fundamentalScore===null?'Technical only':`${metrics.fundamentalScore}/100`}</strong><small>${metrics.fundamentalScore===null?'check results manually':'fundamental component'}</small></div></div></section>`;}function scoreExplanation(metrics){return `<section class="detail-section score-explanation"><p class="research-label">HOW THE SIGNAL IS BUILT</p><h3>${metrics.score}/100 composite research score</h3><div class="score-breakdown"><div><span>Fundamentals</span><strong>${metrics.fundamentalScore===null?'Not available':`${metrics.fundamentalScore}/100`}</strong><small>55% when available: revenue, profit, P/E, ROE and ROCE relative to sector.</small></div><div><span>Technicals</span><strong>${metrics.technicalScore}/100</strong><small>45%: price move, range, 52-week position, EMA trend and RSI.</small></div></div><p class="method-note">If fundamental data is unavailable, the dashboard marks the result as technical-only instead of pretending it is a full company assessment.</p></section>`;}
...
function dynamicDecisionNarrative(metrics){
  ...
  const pattern=metrics.patterns||{},orb=pattern.orb||{},triangle=pattern.triangle||{};if(orb.status==='BREAKOUT CONFIRMED')evidence.push('ORB confirmation is active: '+orb.explanation);else if(orb.status==='FALSE BREAKOUT')evidence.push('ORB warning: '+orb.explanation);else if(orb.status==='BREWING')evidence.push('Opening activity is brewing, but not confirmed: '+orb.explanation);if(triangle.available)evidence.push(triangle.explanation);
...
function renderMarketMovers(view=state.moversView){
  const data=state.moversData,node=$('#movers-list'),note=$('#movers-note'),method=$('#movers-method');
  if(!data||!node)return;
  state.moversView=view;
  document.querySelectorAll('[data-movers-view]').forEach(button=>button.classList.toggle('is-active',button.dataset.moversView===view));
  const rows=view==='broad'?data.broadMovers:view==='candidates'?data.tradeCandidates:data.gainers;
  const title=view==='broad'?'stocks moving at least ±5%':view==='candidates'?'stocks passing all four review gates':'official broad-universe gainers';
  if(note)note.textContent=`${rows.length} ${title} · ${data.source}${data.delayed?' · saved result':''}`;
  if(method)method.textContent=view==='candidates'?data.method:'Ranked from the selected official NSE universe. A daily price rise alone is not a buy signal.';
  node.innerHTML=rows.length?rows.map((stock,index)=>`<article class="mover-row ${stock.candidate?'is-candidate':''}"><div class="mover-rank">#${index+1}</div><div class="mover-company"><strong>${safe(stock.symbol)}</strong><small>${safe(stock.name||stock.symbol)}</small></div><div><span>Last price</span><strong>${money.format(stock.lastPrice)}</strong></div><div><span>Today</span><strong class="${stock.pChange>=0?'positive':'negative'}">${percent(stock.pChange)}</strong></div><div><span>Volume</span><strong>${Number(stock.volume||0).toLocaleString('en-IN')}</strong></div>${view==='candidates'?`<details class="mover-proof"><summary>${stock.candidateScore}/100 · Why it passed</summary><ul>${(stock.reasons||[]).map(reason=>`<li>${safe(reason)}</li>`).join('')}</ul><p>Next step: open Review and confirm the 90-session trend, company news, market context, entry level and stop before considering a trade.</p></details>`:''}<div class="stock-actions"><button class="review-button" data-review="${safe(stock.symbol)}" type="button">Review</button><button class="decision-button" data-decision="${safe(stock.symbol)}" type="button">Decision</button>${watchButtonMarkup(stock.symbol)}</div></article>`).join(''):`<p class="card-message"><strong>No stock meets this view right now.</strong><br>This is a useful result—do not lower the rules merely to produce a candidate.</p>`;
}
async function loadMarketMovers(force=false){
  const node=$('#movers-list'),note=$('#movers-note');
  if(!node)return;
  if(state.moversLoaded&&!force){renderMarketMovers();return;}
  node.innerHTML='<p class="card-message">Loading the official NSE broad-market universe…</p>';
  try{
    const data=await api('movers');
    state.moversData=data;state.moversLoaded=true;
    renderMarketMovers(state.moversView);
    const sources=$('#movers-sources');
    if(sources)sources.innerHTML=(data.comparisonSources||[]).map(source=>`<a href="${safe(source.url)}" target="_blank" rel="noopener noreferrer">${safe(source.label)}</a>`).join('');
  }catch(error){
    if(note)note.textContent='Broad-market data temporarily unavailable';
    node.innerHTML=`<p class="card-message">Could not load broad-market movers: ${safe(error.message)}. <button class="review-button" id="retry-movers" type="button">Retry</button></p>`;
  }
}
function lotPlannerMarkup(){return `<section class="lot-planner-card"><div class="section-title"><div><p class="eyebrow">ALGORITHMIC POSITION & LOT PLANNER</p><h2>Size the trade before taking the trade</h2></div><span>Upstox price + hard risk rules</span></div><p class="recommendations-note">The algorithm uses the smallest quantity allowed by capital, exposure and stop-loss risk. Derivatives are rounded down to complete lots. ML can explain the setup, but it cannot override a failed risk rule.</p><section class="lot-top-section"><div class="lot-top-heading"><div><p class="research-label">CURRENT TOP 10 MARKET MOVERS</p><h3>Start from a mover or search any NSE equity</h3></div><small>Movement is not a buy recommendation. Share quantity or derivative lot count appears after you select a stock and provide the stop-loss and risk limits.</small></div><div id="lot-top-list" class="lot-top-list"><p class="card-message">Loading current Top 10…</p></div></section><form id="lot-form" class="lot-form"><label>Instrument type<select id="lot-instrument-type"><option value="equity">NSE equity</option><option value="futures">Futures</option><option value="options">Options</option></select></label><label>NSE ticker<input id="lot-ticker" placeholder="e.g. TCS" autocomplete="off" required></label><label>Entry price (₹) <span class="field-help" tabindex="0" title="The planned purchase price. Leave blank to load the latest Upstox price.">?</span><input id="lot-entry" type="number" min="0.01" step="0.01" placeholder="Loaded from Upstox"></label><label>Stop-loss (₹) <span class="field-help" tabindex="0" title="The price where the original trade idea is considered wrong. It limits the planned loss; it is not guaranteed during a price gap.">?</span><input id="lot-stop" type="number" min="0.01" step="0.01" required></label><label>Trading capital (₹) <span class="field-help" tabindex="0" title="The total money available for trading. The algorithm will never size this single position above it.">?</span><input id="lot-capital" type="number" min="1000" step="1000" value="100000" required></label><label>Maximum loss (₹) <span class="field-help" tabindex="0" title="The most you plan to lose if price reaches the stop. Quantity is reduced so the estimated stop loss stays within this amount.">?</span><input id="lot-risk" type="number" min="50" step="50" value="1000" required></label><label>Maximum capital exposure (%) <span class="field-help" tabindex="0" title="The largest percentage of your trading capital allowed in one stock. A 25% limit spreads capital instead of concentrating everything in one position.">?</span><input id="lot-exposure" type="number" min="1" max="100" step="1" value="25" required></label><label id="lot-size-label" class="lot-size-field">Exchange lot size<input id="lot-size" type="number" min="1" step="1" value="1" required><small>Equity uses one share. For F&O, verify the current exchange lot size.</small></label><button class="primary-button" id="lot-submit" type="submit">Run lot algorithm</button></form><div id="lot-result" class="lot-result"><p class="card-message">Enter a ticker and stop-loss to calculate a risk-controlled position.</p></div><p class="lot-algo-note"><strong>Algorithm logic:</strong> calculate the quantity allowed by available capital, the selected maximum capital exposure, and the maximum loss between entry and stop-loss; use the smallest quantity; round derivatives down to complete exchange lots; then apply the research-signal gate. ML, ORB and triangle evidence explain confidence but can never increase quantity beyond these hard limits or turn a failed risk rule into a trade.</p></section>`;}
function renderLotTop10(){const node=$('#lot-top-list');if(!node)return;const rows=(state.leaders||[]).slice(0,10);node.innerHTML=rows.length?rows.map((stock,index)=>`<article class="lot-top-card"><span class="stock-rank">#${index+1}</span><div><strong>${safe(stock.symbol)}</strong><small>${money.format(stock.lastPrice)} · <span class="${Number(stock.pChange)>=0?'positive':'negative'}">${percent(stock.pChange)}</span></small></div><div><strong>${safe(stock.signal||'REVIEW')}</strong><small>${Number(stock.score||0)}/100 · ${safe(stock.risk||'risk pending')}</small></div><button class="review-button" data-lot-symbol="${safe(stock.symbol)}" data-lot-price="${Number(stock.lastPrice||0)}" type="button">Plan lot</button><button class="decision-button" data-decision="${safe(stock.symbol)}" type="button">Decision</button></article>`).join(''):'<p class="card-message">Top 10 data is not ready. Press Refresh market data, then return to this tab.</p>';}function lotDecision(metrics,plan){if(!plan.quantity)return {label:'NO AFFORDABLE LOT',tone:'reduce',text:`One complete lot requires more capital or risk than your limits allow. Do not round upward.`};if(metrics.signal==='REDUCE')return {label:'NO NEW ENTRY',tone:'reduce',text:`The quantity fits your budget, but the research signal is REDUCE. Preserve the calculation for learning; wait for the evidence to improve.`};if(metrics.signal==='HOLD')return {label:'WAIT FOR CONFIRMATION',tone:'hold',text:`Risk sizing is valid, but the model is HOLD. Use the Review and Decision buttons to see the missing confirmation.`};return {label:'RISK-CONTROLLED SETUP',tone:'buy',text:`The model signal and hard position-size rules are aligned. Entry still depends on the price trigger and completed-candle confirmation.`};}
function calculateLotPlan(metrics,values){const entry=Number(values.entry||metrics.lastPrice),stop=Number(values.stop),capital=Number(values.capital),riskBudget=Number(values.risk),exposurePct=Number(values.exposure),lotSize=values.type==='equity'?1:Math.max(1,Math.floor(Number(values.lotSize)||0)),riskPerUnit=entry-stop,capitalQty=entry>0?Math.floor(capital/entry):0,exposureQty=entry>0?Math.floor(capital*(exposurePct/100)/entry):0,riskQty=riskPerUnit>0?Math.floor(riskBudget/riskPerUnit):0,rawQty=Math.max(0,Math.min(capitalQty,exposureQty,riskQty)),lots=Math.floor(rawQty/lotSize),quantity=lots*lotSize,capitalUsed=quantity*entry,riskAtStop=quantity*Math.max(0,riskPerUnit),reasons=[];if(!(riskPerUnit>0))reasons.push('Stop-loss must be below the planned entry for a long trade.');if(capitalQty===rawQty)reasons.push('Total available capital is the tightest limit.');if(exposureQty===rawQty)reasons.push(`The ${exposurePct}% portfolio-exposure cap is the tightest limit.`);if(riskQty===rawQty)reasons.push(`The ₹${riskBudget.toLocaleString('en-IN')} maximum-loss rule is the tightest limit.`);if(values.type!=='equity'&&rawQty%lotSize)reasons.push(`${rawQty} units were rounded down to ${quantity} because only complete lots of ${lotSize} are permitted.`);if(quantity<lotSize)reasons.push(`Even one lot of ${lotSize} does not fit all three limits.`);return {entry,stop,capital,riskBudget,exposurePct,lotSize,riskPerUnit,capitalQty,exposureQty,riskQty,rawQty,lots,quantity,capitalUsed,riskAtStop,reasons,type:values.type};}
function renderLotPlan(metrics,plan){
  const node=$('#lot-result');if(!node)return;const decision=lotDecision(metrics,plan),ml=metrics.ml||{},patterns=metrics.patterns||{},triangle=patterns.triangle||{},orb=patterns.orb||{},valid=plan.quantity>0&&plan.riskPerUnit>0,evidence=evidenceFor(metrics),lotValue=plan.entry*plan.lotSize,oneLotRisk=Math.max(0,plan.riskPerUnit)*plan.lotSize,binding=plan.rawQty===plan.exposureQty?'portfolio exposure':plan.rawQty===plan.riskQty?'maximum loss at the stop':plan.rawQty===plan.capitalQty?'available capital':'combined limits',emaText=metrics.ema20>=metrics.ema50?`EMA 20 ${money.format(metrics.ema20)} is above EMA 50 ${money.format(metrics.ema50)}, supporting the recent trend.`:`EMA 20 ${money.format(metrics.ema20)} is below EMA 50 ${money.format(metrics.ema50)}, so shorter-term structure is weak.`,mlText=ml.available?`The five-session ML cross-check estimates ${ml.up}% rise versus ${ml.down}% fall with ${ml.accuracy}% historical test accuracy. This is supporting evidence, not a prediction.`:'ML does not have enough usable history for this stock, so it did not influence the position size.',patternText=orb.available?`ORB is ${orb.status}; ${orb.explanation}`:triangle.available?`${triangle.type} is ${triangle.status.toLowerCase()} with ${triangle.confidence}% structural confidence.`:'No confirmed ORB or triangle evidence is available, so the algorithm does not award pattern confirmation.',nextStep=!valid?'Reduce the lot size only if the exchange contract permits it; otherwise increase neither capital nor risk merely to force a trade.':metrics.signal==='BUY'?`Confirm entry near ${money.format(plan.entry)} and keep the invalidation at ${money.format(plan.stop)}. If price gaps through the stop, actual loss can be larger.`:`Do not enter from sizing alone. Open Decision and wait for the missing price, trend or model confirmation.`;
  const isEquity=plan.type==='equity',positionSummary=isEquity?`<section class="position-summary"><p class="research-label">YOUR POSITION IN SIMPLE WORDS</p><h3>Buy ${plan.quantity||0} shares at ${money.format(plan.entry)} per share</h3><p>The complete position would cost <strong>${money.format(plan.capitalUsed)}</strong>. If price reaches the stop-loss at ${money.format(plan.stop)}, the estimated loss is <strong>${money.format(plan.riskAtStop)}</strong> (${money.format(plan.riskPerUnit)} per share).</p></section>`:`<section class="position-summary"><p class="research-label">YOUR CONTRACT POSITION IN SIMPLE WORDS</p><h3>${plan.lots} lots × ${plan.lotSize} units = ${plan.quantity} total units</h3><p>Each unit is priced at ${money.format(plan.entry)}. One complete lot has an estimated contract ${plan.type==='options'?'premium':'notional value'} of <strong>${money.format(lotValue)}</strong>; all selected lots total <strong>${money.format(plan.capitalUsed)}</strong>. ${plan.type==='futures'?'This is contract value, not the broker margin required.':''}</p></section>`,instrumentCards=isEquity?`<div class="lot-metrics equity-position-metrics"><div class="lot-count-highlight"><span>Number of shares</span><strong>${plan.quantity||0}</strong><small>cash equity has no mandatory F&O lot</small></div><div><span>Price per share</span><strong>${money.format(plan.entry)}</strong><small>planned entry price</small></div><div><span>Total position cost</span><strong>${money.format(plan.capitalUsed)}</strong><small>${plan.quantity||0} × ${money.format(plan.entry)}</small></div><div><span>Stop-loss price</span><strong>${money.format(plan.stop)}</strong><small>where the long idea is treated as wrong</small></div><div><span>Loss per share at stop</span><strong>${money.format(plan.riskPerUnit)}</strong><small>entry minus stop</small></div><div><span>Estimated total loss</span><strong>${money.format(plan.riskAtStop)}</strong><small>selected maximum ${money.format(plan.riskBudget)}</small></div><div><span>Capital exposure</span><strong>${plan.capital?`${(plan.capitalUsed/plan.capital*100).toFixed(1)}%`:'—'}</strong><small>${money.format(plan.capitalUsed)} of ${money.format(plan.capital)}</small></div></div>`:`<div class="lot-metrics derivative-position-metrics"><div class="lot-count-highlight"><span>Number of lots</span><strong>${plan.lots}</strong><small>${plan.lots?`${plan.lots} complete contract lot${plan.lots===1?'':'s'}`:'no complete lot fits'}</small></div><div><span>Units in each lot</span><strong>${plan.lotSize}</strong><small>verify for the selected contract and expiry</small></div><div><span>Total units</span><strong>${plan.quantity||0}</strong><small>${plan.lots} × ${plan.lotSize}</small></div><div><span>Price per unit</span><strong>${money.format(plan.entry)}</strong><small>${plan.type==='options'?'option premium per unit':'futures price per unit'}</small></div><div><span>${plan.type==='options'?'Premium for one lot':'Notional value of one lot'}</span><strong>${money.format(lotValue)}</strong><small>${plan.lotSize} × ${money.format(plan.entry)}</small></div><div><span>Total ${plan.type==='options'?'premium':'contract value'}</span><strong>${money.format(plan.capitalUsed)}</strong><small>${plan.lots} complete lots</small></div><div><span>Estimated stop risk</span><strong>${money.format(plan.riskAtStop)}</strong><small>${money.format(oneLotRisk)} per lot</small></div></div>`,calculationText=isEquity?`<p><strong>The binding rule is ${safe(binding)}.</strong> The algorithm permits ${plan.capitalQty} shares by total capital, ${plan.exposureQty} shares by the ${plan.exposurePct}% exposure cap, and ${plan.riskQty} shares by the ₹${plan.riskBudget.toLocaleString('en-IN')} stop-loss budget. It takes the smallest number: <strong>${plan.quantity} shares</strong>.</p>`:`<p><strong>The binding rule is ${safe(binding)}.</strong> The algorithm permits ${plan.capitalQty} units by total capital, ${plan.exposureQty} units by exposure, and ${plan.riskQty} units by stop-loss risk. It takes the smallest result, then rounds down to complete ${plan.lotSize}-unit lots: <strong>${plan.lots} lots / ${plan.quantity} units</strong>.</p>`;
  state.lotPlan={metrics,plan};node.innerHTML=`<article class="lot-output"><div class="decision-hero"><span class="signal ${safe(decision.tone)}">${safe(decision.label)}</span><h3>${safe(metrics.symbol)} · ${isEquity?`${plan.quantity||0} shares`:`${plan.lots} complete lot${plan.lots===1?'':'s'}`}</h3><p>${safe(decision.text)}</p></div>${positionSummary}${instrumentCards}<section class="lot-explanation"><p class="research-label">ALGORITHM VERDICT — STOCK-SPECIFIC EVIDENCE</p><h3>Why this is ${safe(decision.label.toLowerCase())} for ${safe(metrics.symbol)}</h3><div class="lot-evidence-grid"><div><h4>Price and trend evidence</h4><ul><li>${safe(emaText)}</li><li>The 20-session move is ${percent(metrics.trend)}; RSI is ${valueOrDash(metrics.rsi)}.</li><li>Support is ${money.format(metrics.support)} and resistance is ${money.format(metrics.resistance)}. Your stop is ${money.format(plan.stop)}.</li></ul></div><div><h4>ML and pattern evidence</h4><ul><li>${safe(mlText)}</li><li>${safe(patternText)}</li><li>Pattern confirmation score is ${Number(patterns.score||0)}/100 and never overrides the stop rule.</li></ul></div><div><h4>Evidence supporting the setup</h4><ul>${evidence.positive.slice(0,4).map(item=>`<li>${safe(item)}</li>`).join('')||'<li>No strong positive evidence is currently confirmed.</li>'}</ul></div><div><h4>Evidence requiring caution</h4><ul>${evidence.caution.slice(0,4).map(item=>`<li>${safe(item)}</li>`).join('')||'<li>No additional model caution was detected; normal market risk remains.</li>'}</ul></div></div></section><section class="lot-proof"><h4>Why the algorithm chose this quantity</h4>${calculationText}<ul>${plan.reasons.map(reason=>`<li>${safe(reason)}</li>`).join('')||'<li>All hard limits permit the displayed quantity.</li>'}</ul><details><summary>What do stop-loss and maximum exposure mean?</summary><p><strong>Stop-loss:</strong> ${money.format(plan.stop)} is where the long-trade idea is treated as wrong. The estimated loss assumes an execution at that price; gaps and slippage can make the real loss larger.</p><p><strong>Maximum capital exposure:</strong> ${plan.exposurePct}% means this one position may use no more than ${money.format(plan.capital*plan.exposurePct/100)} of the ${money.format(plan.capital)} trading capital. It reduces concentration risk.</p></details></section><section class="lot-next-step"><h4>Practical next step</h4><p>${safe(nextStep)}</p></section><div class="stock-card-actions"><button class="review-button" data-review="${safe(metrics.symbol)}" type="button">Full Review</button><button class="decision-button" data-decision="${safe(metrics.symbol)}" type="button">Decision with evidence</button>${valid&&plan.type==='equity'?`<button class="paper-enter" data-lot-paper="${safe(metrics.symbol)}" type="button">Use for paper entry</button>`:''}</div><p class="method-note">Educational sizing only. F&O lots, expiries, margins and contract specifications can change. Confirm them on Upstox/NSE. Brokerage, taxes, slippage, gaps and option Greeks are not included.</p></article>`;
}function setupLotPlanner(panel){if(!panel||panel.dataset.ready)return;panel.dataset.ready='true';panel.innerHTML=lotPlannerMarkup();const type=$('#lot-instrument-type'),lot=$('#lot-size');type.addEventListener('change',()=>{const equity=type.value==='equity';lot.value=equity?'1':lot.value;lot.disabled=equity;$('#lot-size-label small').textContent=equity?'NSE cash equities are sized in whole shares.':'Enter and verify the current exchange contract lot size from Upstox/NSE.';});lot.disabled=true;$('#lot-form').addEventListener('submit',async event=>{event.preventDefault();const ticker=cleanTicker($('#lot-ticker').value),button=$('#lot-submit'),node=$('#lot-result');if(!ticker)return;button.disabled=true;button.textContent='Running algorithm…';node.innerHTML=`<p class="card-message">Loading ${safe(ticker)} price, trend, ML and risk evidence…</p>`;try{const metrics=await loadStockAnalysis(ticker),entry=Number($('#lot-entry').value)||metrics.lastPrice;$('#lot-entry').value=entry.toFixed(2);const plan=calculateLotPlan(metrics,{type:type.value,entry,stop:$('#lot-stop').value,capital:$('#lot-capital').value,risk:$('#lot-risk').value,exposure:$('#lot-exposure').value,lotSize:lot.value});renderLotPlan(metrics,plan);}catch(error){node.innerHTML=`<p class="card-message">Could not build the lot plan: ${safe(error.message)}</p>`;}finally{button.disabled=false;button.textContent='Run lot algorithm';}});panel.addEventListener('click',event=>{const pick=event.target.closest('[data-lot-symbol]');if(pick){$('#lot-ticker').value=pick.dataset.lotSymbol;$('#lot-entry').value=Number(pick.dataset.lotPrice||0).toFixed(2);const leader=state.leaders.find(row=>row.symbol===pick.dataset.lotSymbol);if(leader?.dayLow)$('#lot-stop').value=Number(leader.dayLow).toFixed(2);$('#lot-stop').focus();return;}const paper=event.target.closest('[data-lot-paper]');if(!paper||!state.lotPlan)return;const {metrics,plan}=state.lotPlan;let item=state.watchlist.find(row=>row.symbol===metrics.symbol);if(!item){item={symbol:metrics.symbol,target:null,stop:plan.stop};saveWatchlist([...state.watchlist,item]);}else if(!item.stop){item.stop=plan.stop;saveWatchlist([...state.watchlist]);}state.watchRows=[...state.watchRows.filter(row=>row.symbol!==metrics.symbol),{symbol:metrics.symbol,metrics,fetchedAt:new Date().toISOString()}];openPaperEntryDialog(metrics.symbol,plan.entry);const quantity=$('#paper-entry-quantity');if(quantity)quantity.value=String(plan.quantity);});}
function activateWorkspaceTab(name){document.querySelectorAll('[data-tab]').forEach(button=>{const active=button.dataset.tab===name;button.classList.toggle('is-active',active);button.setAttribute('aria-selected',String(active));});document.querySelectorAll('.tab-panel').forEach(panel=>{const active=panel.id===`panel-${name}`;panel.classList.toggle('is-active',active);panel.hidden=!active;});if(name==='recommended'){void loadRecommendations();void loadIntradaySetups();}if(name==='sectors'&&!state.sectorLoaded)void loadSector();if(name==='updates')void loadUpdates();if(name==='lots')renderLotTop10();if(name==='watchlist')void refreshWatchlist();}function setupWorkspace(){const main=document.querySelector('main'),hero=main?.querySelector('.hero');if(!main||!hero||document.querySelector('.workspace-tabs'))return;main.classList.add('app-shell');hero.classList.add('workspace-header');const eyebrow=hero.querySelector('.eyebrow'),title=hero.querySelector('h1'),description=hero.querySelector('p:not(.eyebrow)'),refreshButton=hero.querySelector('#refresh');if(eyebrow)eyebrow.textContent='INDIAN MARKET RESEARCH';if(title)title.textContent='Market Pulse research dashboard';if(description)description.textContent='Multi-factor signals, focused decisions, live market context and practical research tools.';if(refreshButton)refreshButton.textContent='Refresh data';const tabs=document.createElement('nav');tabs.className='workspace-tabs';tabs.setAttribute('role','tablist');tabs.setAttribute('aria-label','Market Pulse sections');const definitions=[['market','Top 10'],['recommended','Recommended'],['sectors','Sectors'],['research','Research'],['updates','Updates'],['lots','Lot Planner'],['watchlist','Watchlist']];tabs.innerHTML=definitions.map(([key,label],index)=>`<button class="workspace-tab${index===0?' is-active':''}" id="tab-${key}" role="tab" aria-selected="${index===0}" aria-controls="panel-${key}" data-tab="${key}" type="button">${label}</button>`).join('');const panels=document.createElement('div');panels.className='tab-panels';const makePanel=(key,label)=>{const panel=document.createElement('section');panel.className=`tab-panel${key==='market'?' is-active':''}`;panel.id=`panel-${key}`;panel.setAttribute('role','tabpanel');panel.setAttribute('aria-labelledby',`tab-${key}`);panel.hidden=key!=='market';panel.dataset.label=label;panels.append(panel);return panel;};const market=makePanel('market','Top 10'),recommended=makePanel('recommended','Recommended'),sectors=makePanel('sectors','Sectors'),research=makePanel('research','Research'),updates=makePanel('updates','Updates'),lots=makePanel('lots','Lot Planner'),watchlist=makePanel('watchlist','Watchlist');['.disclaimer','.metric-grid','.screening'].forEach(selector=>{const node=main.querySelector(selector);if(node)market.append(node);});const marketTable=market.querySelector('.table-wrap');if(marketTable){marketTable.insertAdjacentHTML('beforebegin','<div id="stock-pager" class="data-pager" aria-label="Top 10 pages"></div>');marketTable.insertAdjacentHTML('afterend','<div id="stock-cards" class="stock-card-list" aria-live="polite"></div>');}const movers=document.createElement('section');movers.className='movers-card';movers.innerHTML='<div class="section-title"><div><p class="eyebrow">MARKET MOVERS LAB</p><h2>Official gainers, broad movers and review candidates</h2></div><span id="movers-note">Loading official NSE data…</span></div><div class="movers-tabs" role="tablist"><button class="movers-tab is-active" data-movers-view="gainers" type="button">NSE top gainers</button><button class="movers-tab" data-movers-view="broad" type="button">Broad ±5% movers</button><button class="movers-tab" data-movers-view="candidates" type="button">Trade candidates</button></div><p id="movers-method" class="recommendations-note">Daily gainers are separated from stocks that pass practical review gates.</p><div id="movers-list" class="movers-list" aria-live="polite"></div><div id="movers-sources" class="movers-sources"></div></section>';market.append(movers);movers.addEventListener('click',event=>{const tab=event.target.closest('[data-movers-view]');if(tab)return renderMarketMovers(tab.dataset.moversView);if(event.target.closest('#retry-movers'))void loadMarketMovers(true);});recommended.innerHTML='<section class="recommendations-card"><div class="section-title"><div><p class="eyebrow">RECOMMENDED BY MARKET PULSE</p><h2>Up to 3 buy-qualified candidates</h2></div><span>Updated from current market data</span></div><p id="recommendations-note" class="recommendations-note">Only stocks that pass every BUY-quality gate are shown. Fewer than three appear when current evidence is not strong enough.</p><div id="recommendation-list" class="recommendation-list"><p class="card-message">Open this tab to build the latest shortlist.</p></div><p id="recommendations-allocation" class="method-note"></p></section><section class="recommendations-card intraday-section"><div class="section-title"><div><p class="eyebrow">SAME-DAY PATTERN SCREEN</p><h2>Up to 3 intraday setups</h2></div><span>Live 5-minute candles</span></div><p id="intraday-note" class="recommendations-note">Only confirmed VWAP, EMA, opening-range, volume and reward/risk setups appear. No setup means no trade.</p><div id="intraday-list" class="recommendation-list"><p class="card-message">Open this tab to check current intraday patterns.</p></div></section>';['.lookup-card','.investment-card'].forEach(selector=>{const node=main.querySelector(selector);if(node)research.append(node);});const lookupPane=research.querySelector('.lookup-card'),calculatorPane=research.querySelector('.investment-card');if(lookupPane&&calculatorPane){lookupPane.classList.add('research-pane','is-active');calculatorPane.classList.add('research-pane');calculatorPane.hidden=true;const researchTabs=document.createElement('div');researchTabs.className='research-tabs';researchTabs.innerHTML='<button class="research-tab is-active" data-research-pane="lookup" type="button">Ticker analysis</button><button class="research-tab" data-research-pane="calculator" type="button">Return calculator</button>';research.prepend(researchTabs);researchTabs.addEventListener('click',event=>{const button=event.target.closest('[data-research-pane]');if(!button)return;const calculator=button.dataset.researchPane==='calculator';lookupPane.hidden=calculator;calculatorPane.hidden=!calculator;researchTabs.querySelectorAll('button').forEach(tab=>tab.classList.toggle('is-active',tab===button));});}const sectorContent=document.createElement('div');sectorContent.className='sector-workspace';sectorContent.innerHTML=`<div class="section-title sector-heading"><div><p class="eyebrow">SECTOR MOMENTUM</p><h2>Top 10 stocks by sector</h2></div><span id="sector-note">Choose a sector to load the latest ranking.</span></div><div class="sector-tabs" role="tablist" aria-label="NSE sector ranking">${Object.entries(SECTOR_LABELS).map(([key,label],index)=>`<button class="sector-tab${index===0?' is-active':''}" data-sector="${key}" type="button">${label}</button>`).join('')}</div><div class="sector-insight" id="sector-insight" aria-live="polite">Sector analysis uses current NSE-symbol data. It is research, not a price target.</div><div class="table-wrap sector-table-wrap"><table><thead><tr><th>#</th><th>Company</th><th>Last price</th><th>Today</th><th>3M trend</th><th>Signal</th><th>12M scenario</th><th>Trade levels</th><th>Actions</th></tr></thead><tbody id="sector-table"></tbody></table></div>`;sectors.append(sectorContent);const sectorTable=sectors.querySelector('.sector-table-wrap');if(sectorTable){sectorTable.insertAdjacentHTML('beforebegin','<div id="sector-pager" class="data-pager" aria-label="Sector ranking pages"></div>');sectorTable.insertAdjacentHTML('afterend','<div id="sector-cards" class="stock-card-list" aria-live="polite"></div>');}const updatesCard=main.querySelector('.updates-card');if(updatesCard)updates.append(updatesCard);const watchlistCard=main.querySelector('.watchlist-card');if(watchlistCard){watchlist.append(watchlistCard);watchlistCard.querySelector('#watchlist-items')?.insertAdjacentHTML('afterend','<div id="watch-pager" class="data-pager" aria-label="Watchlist pages"></div>');}hero.after(tabs,panels);setupLotPlanner(lots);void loadMarketMovers();tabs.addEventListener('click',event=>{const button=event.target.closest('[data-tab]');if(button)activateWorkspaceTab(button.dataset.tab);});}
...
document.addEventListener('click', event => {
  ...
  if (pageButton && !pageButton.disabled) {
    ...
    if (pageButton.dataset.pageKind === 'leaders') { state.leaderPage = next; renderLeaders();renderLotTop10(); }
  ...
  if (watch) { event.preventDefault(); event.stopPropagation(); return toggleWatch(watch.dataset.watch); }
  ...
  if (remove) { event.preventDefault(); event.stopPropagation(); return stopWatching(remove.dataset.removeWatch); }
});
...
window.addEventListener('resize', () => {
  ...
  resizeTimer = window.setTimeout(() => {
    if (state.leaders.length) renderLeaders();renderLotTop10();
...
$('#version-toggle')?.addEventListener('click',()=>changelogModal?.showModal());
$('#changelog-close')?.addEventListener('click',()=>changelogModal?.close());
changelogModal?.addEventListener('click',event=>{if(event.target===changelogModal)changelogModal.close();});
function setupTickerAutocomplete(inputSelector='#ticker-input',formSelector='#ticker-form'){
  const input=$(inputSelector),form=$(formSelector);if(!input||!form||form.querySelector('.ticker-suggestions'))return;const box=document.createElement('div');box.className='ticker-suggestions';box.id=`${input.id}-suggestions`;box.setAttribute('role','listbox');box.hidden=true;form.append(box);input.setAttribute('role','combobox');input.setAttribute('aria-controls',box.id);input.setAttribute('aria-expanded','false');let timer=0,request=0,active=-1,matches=[];
  const close=()=>{box.hidden=true;box.innerHTML='';input.setAttribute('aria-expanded','false');active=-1;};
  const choose=item=>{if(!item)return;input.value=item.symbol;input.dataset.selectedSymbol=item.symbol;close();input.focus();};
...
setupWorkspace();
setupTickerAutocomplete('#ticker-input','#ticker-form');
setupTickerAutocomplete('#investment-ticker','#investment-form');
setupTickerAutocomplete('#watch-ticker','#watchlist-form');
setupTickerAutocomplete('#lot-ticker','#lot-form');
